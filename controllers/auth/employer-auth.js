...
class EmployerAuthController {
	...
	static async signup(req, res, next) {
		validateFields(req, ["email", "password", "companyName"]);
		...
		if (findEmp) {
			return next(
				new AppError("Employer with this email already exists", 404)
			);
		}
	...
	static async login(req, res, next) {
		...
		if (!emp) {
			return next(
				new AppError(`No employer found with the email: ${email}`, 404)
			);
		}
	...
	static async forgotPassword(req, res, next) {
		...
		if (!emp) {
			return next(
				new AppError("Employer with this email does not exist", 404)
			);
		}
